...
COVERS: dict[str, tuple[TuyaCoverEntityDescription, ...]] = {
    ...
    # HomeGate sliding gate operator
    # Not documented in official Tuya documentation
    "qt": (
        TuyaCoverEntityDescription(
            key=DPCode.GATE_OPEN,
            translation_key="gate",
            current_state=DPCode.GATE_OPEN_TRAVEL,
            device_class=CoverDeviceClass.GATE,
            open_instruction_value=True,
            close_instruction_value=False,
            stop_instruction_value=DPCode.GATE_STOP,
        ),
        TuyaCoverEntityDescription(
            key=DPCode.GATE_CLOSE,
            translation_key="gate",
            current_state=DPCode.GATE_CLOSE_TRAVEL,
            current_state_inverse=True,
            device_class=CoverDeviceClass.GATE,
            open_instruction_value=False,
            close_instruction_value=True,
            stop_instruction_value=DPCode.GATE_STOP,
        ),
    ),
}
...
class TuyaCoverEntity(TuyaEntity, CoverEntity):
    ...
    def stop_cover(self, **kwargs: Any) -> None:
        """Stop the cover."""
        if isinstance(self.entity_description.stop_instruction_value, DPCode):
            # Для устройства HomeGate, которое использует отдельную DP команду для остановки
            self._send_command([{"code": self.entity_description.stop_instruction_value, "value": True}])
        else:
            # Для всех остальных устройств, использующих стандартный механизм
            self._send_command(
                [
                    {
